package com.example.buivanminh.util;

import com.example.buivanminh.dto.response.*;
import com.example.buivanminh.entity.Order;
import com.example.buivanminh.entity.OrderItem;
import com.example.buivanminh.entity.OrderTimeline;
import com.example.buivanminh.repository.OrderItemRepository;
import com.example.buivanminh.repository.OrderTimelineRepository;
...
public class OrderMapper {
        ...
        public OrderResponseDTO toOrderResponseDTO(Order order) {
                ...
                responseDTO.setPaymentMethodId(
                                order.getPaymentMethod() != null ? order.getPaymentMethod().getId() : null);
                responseDTO
                                .setShippingAddressId(
                                                order.getShippingAddress() != null ? order.getShippingAddress().getId()
                                                                : null);

                // Lấy OrderItems từ database
                List<OrderItem> items = orderItemRepository.findByOrderId(order.getId());
                logger.info("Found " + items.size() + " OrderItems for order: " +
                                order.getId());
                ...
                logger.info("Found " + timelines.size() + " OrderTimelines for order: " +
                                order.getId());
        ...
        public OrderItemResponseDTO toOrderItemResponseDTO(OrderItem orderItem) {
                logger.info("Mapping OrderItem to OrderItemResponseDTO: " +
                                orderItem.getId());
        ...
        public OrderTimelineResponseDTO toOrderTimelineResponseDTO(OrderTimeline timeline) {
                logger.info("Mapping OrderTimeline to OrderTimelineResponseDTO: " +
                                timeline.getId());
