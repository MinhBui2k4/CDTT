...
import { GET_PRODUCT_BY_ID, UPDATE_PRODUCT, GET_ALL_CATEGORIES_ADMIN, GET_ALL_BRANDS } from "../../../api/adminApiService";
...
export default function ProductEdit(): JSX.Element {
  ...
  const [imageFields, setImageFields] = useState<number[]>([]);
    ...
    const fetchData = async (): Promise<void> => {
      ...
      try {
        ...
        setFormData({
          ...
          review: productData.review.toString() || "",
          image: null,
          images: [],
          ...
          sku: productData.sku || "",
          availability: productData.quantity === 0 ? "false" : productData.availability.toString() || "true",
          new: productData.new.toString() || "false",
          sale: productData.sale.toString() || "false",
        });

        setImageFields(productData.images ? Array(productData.images.length).fill(0).map((_, i) => i) : [0]);
        setMainImagePreview(productData.image ? `http://localhost:8080/api/products/image/${productData.image}` : null);
        setAdditionalImagePreviews(
          productData.images ? productData.images.map((img) => `http://localhost:8080/api/products/images/${img}`) : []
        );
  ...
  if (loading || !product) return <p className="text-center py-8">Đang tải...</p>;

  return (
    <div className="p-6 max-w-4xl mx-auto bg-white rounded-lg shadow">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold text-blue-600">Chỉnh sửa sản phẩm</h2>
        <button
